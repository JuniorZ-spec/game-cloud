...
locally is risky: losing it makes Terraform "forget" everything it created, and resources
would keep running and costing money with no record of how to find them. Two concurrent
runs could also corrupt the file without a lock. The standard fix is an S3 bucket
...
rejected. This AWS account restricts EC2 instances to Free-Tier-eligible types only, likely
a guardrail on a recent account. I listed the actually available types via `aws ec2
...
`ACTIVE` with `Public: false` (public access confirmed disabled), node group `ACTIVE` with
no issues.
...
immutable tags (`IMMUTABLE`: a SHA tag, once pushed, can never be overwritten, matching the
idea that one tag equals one exact commit) and a lifecycle rule keeping only the 10 most
...
pushing to my 7 ECR repos. No other GitHub project, even on this same AWS account, can use
it.
...
the list of actually touched services. If nothing under `services/` changed, nothing runs.
`build-scan-push` then runs a dynamic matrix over that list: one parallel job per modified
service, never all 7 at once. Each job gets short-lived AWS credentials via the OIDC
bridge, builds the Docker image, scans it with Trivy (the vulnerability report stays
...
landed in `gamecloud/auth-api` tagged `d97bf26d8858a7913e2634b60c1d319d7155db2d`, the exact
SHA of the commit that produced it. The other 6 ECR repos verified empty (`aws ecr
...
technique, built for pulling *remote* charts, not composing a local one repeatedly. I went
with a more standard approach instead: Helm packages the application (one generic
...
trade-off: deployment stays 100% automatic, but the exact deployed tag is no longer visible
in `values-*.yaml` on Git.
...
`v1.2.4`/`v1.2.2`) no longer reads annotations globally as I initially assumed. It needs an
explicit `ImageUpdater` CRD object. Added `argocd/imageupdater.yaml` with
...
throwaway resource with no residual cost.
